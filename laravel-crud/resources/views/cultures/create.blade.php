...
<body>
    <div class="card">
        ...
        <form action="{{ route('cultures.store') }}" method="POST">
            ...
            <input id="emoji" name="emoji" type="text" value="{{ old('emoji') }}" required>
            @error('emoji')<div class="error">{{ $message }}</div>@enderror

            <label for="flag_path">Flag path</label>
            <input id="flag_path" name="flag_path" type="text" value="{{ old('flag_path') }}" required>
